...
export function calculateSuggestedRequirements({ items = [], balances = [], bills = [], outletId }) {
  ...
  return activeItems.map(item => {
    ...
    let quantity;
    if (item.supply_unit) {
      const supplyUnit = item.supply_unit;
      const inventoryUnit = item.inventory_unit || item.base_unit;
      if (supplyUnit === 'KG' && inventoryUnit === 'G') quantity = raw / 1000;
      else if (supplyUnit === 'L' && inventoryUnit === 'ML') quantity = raw / 1000;
      else if ((supplyUnit === 'G' && inventoryUnit === 'G') || (supplyUnit === 'ML' && inventoryUnit === 'ML')) quantity = raw;
      else if (supplyUnit === 'PIECE' && inventoryUnit === 'EACH') quantity = Math.ceil(raw);
      else quantity = 0; // Container contents vary; the owner enters bottles/packs manually.
