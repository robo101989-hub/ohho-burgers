...
async function loadData(auth, req, res) {
  ...
  if (!outletIds.length) return res.status(200).json({ items: itemsResult.data || [], outlets, inventory: [], bills: [], movements: [], notifications: [] });
  ...
  const [inventoryResult, billsResult, movementsResult, notificationsResult] = await Promise.all([inventoryQuery, billsQuery, movementsQuery, notificationsQuery]);
  if (inventoryResult.error || billsResult.error || movementsResult.error || notificationsResult.error) return res.status(500).json({ error: 'Unable to load inventory records.' });
