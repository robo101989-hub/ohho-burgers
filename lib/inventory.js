...
import { measurementColumns, normalizeSupplyLine } from './inventory-measurements.js';
...
async function loadData(auth, req, res) {
  ...
  const [inventoryResult, billsResult, movementsResult, notificationsResult, requestsResult, expensesResult] = await Promise.all([
...
async function updateItem(auth, body, res) {
  ...
  let units;
  try { units = measurementColumns(body.measurementType); }
...
async function saveRecipeIngredient(auth, body, res) {
  ...
  const baseQuantity = item.base_unit === 'G' && item.display_unit === 'KG' ? quantity * 1000 : quantity;
...
async function saveStockRequest(auth, body, res) {
  ...
  const normalized = rows.map(row => {
    ...
    if (unit === 'PIECE') quantity = Math.max(1, Math.round(quantity));
